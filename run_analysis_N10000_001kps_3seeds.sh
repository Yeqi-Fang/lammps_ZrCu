#!/usr/bin/env bash
# Structural analysis for Cu50Zr50 N=10000 quench at 0.01 K/ps (q1e10) seeds.
set -euo pipefail

cd "$(dirname "$0")"

if [[ -f "$HOME/WORK/miniconda3/etc/profile.d/conda.sh" ]]; then
  ...
  echo "Conda shell hook was not found." && exit 127
fi
...
# quench_steps = 170000000 for 0.01 K/ps  (1700 K / 0.01 K/ps / 0.001 ps/step)
...
python aggregate_seed_analyses.py \
  --tags "${TAGS[@]}" \
  --out  analysis_N10000_001kps_3seeds
